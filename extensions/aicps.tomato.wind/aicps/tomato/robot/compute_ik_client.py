...
by the camera-pose sampler: multi-seed IK retry + multi-batch support.

NOTE: sampled_poses entries (from export_sampled_poses.py) are ALREADY
Link6 target poses in base_link frame
"""
import glob
import os
import rclpy
...
def run_batch(sampled_poses, n_random_fallbacks=1, batch_label=""):
   ...
   for i, entry in enumerate(sampled_poses):
       # entry["position_xyz"]/["quat_xyzw"] ARE ALREADY Link6 target
       # poses (in base_link frame) -- export_sampled_poses.py back-solves
       # these directly from the camera's known offset from Link6
       # (CAMERA_LOCAL_TO_LINK6 in camera_pose_sampler_v2.py), which is a
       # separate mechanism from grasp_offset.py. Do NOT run these through
       # grasp_target_to_link6_target() -- they are not grasp-point poses,
       # and doing so would double-shift them. grasp_offset.py is for a
       # different, not-yet-built step: commanding the arm so the
       # GRIPPER's grasp point lands on an actual target (e.g. a fruit),
       # not for this camera-framing pipeline.
...
if __name__ == "__main__":
   ...
   try:
       ...
       if multi_paths:
           ...
           example_poses = [
               {"position_xyz": [0.3, 0.0, 0.3], "quat_xyzw": [0.0, 0.0, 0.0, 1.0]},
               {"position_xyz": [0.35, 0.1, 0.25], "quat_xyzw": [0.0, 0.0, 0.0, 1.0]},
           ]
